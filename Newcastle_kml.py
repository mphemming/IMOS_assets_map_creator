...
for row in inputfile:
    ...
    if n< 6:
        # ATF moorings
        pnt = kml.newpoint(name='Existing ATF moorings', coords=[(row[2],row[1])])        
    ...
    if n >= 6 and n < 8:
        # HF radar sites
        pnt = kml.newpoint(name='Existing HF radar sites', coords=[(row[2],row[1])])              
        pnt.style.iconstyle.color = simplekml.Color.green
        pnt.style.labelstyle.scale = 0 
        pnt.iconstyle.scale = 1        
    ...
    if n >= 16 and n < 19:
        ...
        pnt = kml.newpoint(name='Proposed moorings', coords=[(row[2],row[1])])              
        pnt.style.labelstyle.scale = 0
    ...
    if n >= 19 and n < 21: 
        ...
        pnt = kml.newpoint(name='Proposed HF radar sites', coords=[(row[2],row[1])])   
        pnt.style.labelstyle.scale = 0
        pnt.style.labelstyle.color = simplekml.Color.white          
        pnt.style.iconstyle.color = simplekml.Color.red
        pnt.iconstyle.scale = 1        
    ...
    if n >= 40 and n < 49:
        # Other NSW Moorings
        pnt = kml.newpoint(name='Existing ANMN moorings', coords=[(row[2],row[1])])              
        pnt.style.labelstyle.scale = 0
        pnt.iconstyle.scale = 1        
        pnt.style.iconstyle.color = simplekml.Color.green
        pnt.style.iconstyle.icon.href = 'http://maps.google.com/mapfiles/kml/shapes/placemark_square.png'     
    if n >= 49 and n < 52:
        # Other NSW Moorings
        pnt = kml.newpoint(name='Existing biogeochemical sampling sites', coords=[(row[2],row[1])])              
        pnt.style.labelstyle.scale = 0 
        pnt.iconstyle.scale = 1        
        pnt.style.iconstyle.color = simplekml.Color.green
